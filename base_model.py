# -*- coding: utf-8 -*-
"""
Created on Fri Sep  4 12:09:39 2020
...
class ImageReconstruction_reflection(object):
    ...
    def _build_model(self, images, flow, last = None):

        b = self.batch_size
        h = self.CROP_PATCH_H // (2 ** self.level)
        w = self.CROP_PATCH_W // (2 ** self.level)
        input_images = tf.concat([images[0], images[1], images[2], images[3], images[4]], 3)
        ...
        FB20, FB21, FB23, FB24, \
        FB30, FB31, FB32, FB34, \
        FB40, FB41, FB42, FB43 = flow
        if last != None :
            F0_last, F1_last, F2_last, F3_last, F4_last, B0_last, B1_last, B2_last, B3_last, B4_last = last
...
class Decomposition_Net_Translation(object):
    ...
    def inference(self, Image):
        ...
        Args:
        """
        return self._build_model(Image)
        ...
        for i in range(5):
            for j in range(5):
                if i == j :
                    ...
                    F, B = Estimator(feature[i], feature[j])
                    FF.append(F)
                    FB.append(B)
