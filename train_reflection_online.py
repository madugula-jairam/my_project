...
tf.flags.DEFINE_string('train_dir', 'temp_online_ckpt/',
                          """Directory where to write event logs and checkpoint.""")
tf.flags.DEFINE_integer('max_steps', 1001,
                            """Number of steps to run.""")
tf.flags.DEFINE_integer('batch_size', 1,
                            """The number of samples in each batch.""")
tf.flags.DEFINE_float('initial_learning_rate', 0.0001,
                          """Initial learning rate.""")
tf.flags.DEFINE_string('training_data_path', 'imgs',
                           """Training data path.""")
tf.flags.DEFINE_string('training_scene', '00002',
                           """Training scene id in training data path.""")
tf.flags.DEFINE_integer('blur_size', 21,
    ...
    with tf.Graph().as_default():
        def get_online_data(path):
            ...
            dataset_F0 = dataset_F0.apply(
                tf2.data.experimental.shuffle_and_repeat(buffer_size=21, count=None, seed=6)).map(
