...
_DEFAULT_PWCNET_TRAIN_OPTIONS = {
    'train_directory' : 'temp_online_ckpt',
    'max_steps' : 1001,
    'initial_learning_rate' : 0.0001,
    'training_data_path' : 'input_imgs/',
    'resize_imgs_path' : 'tmp/',
...
class ModelPWCNet(ModelBase):
        ...
        with tf.variable_scope(name):
            for pyr, x, reuse, name in zip([c1, c2], [x_tnsr[:, 0], x_tnsr[:, 1]], [None, True], ['c1', 'c2']):
                for lvl in range(1, self.opts['pyr_lvls'] + 1):
                    ...
                    f = num_chann[lvl]
                    x = tf.layers.conv2d(x, f, 3, 2, 'same', kernel_initializer=init, name=f'conv{lvl}a', reuse=tf.AUTO_REUSE)
                    x = tf.nn.leaky_relu(x, alpha=0.1)  # , name=f'relu{lvl+1}a') # default alpha is 0.2 for TF
                    x = tf.layers.conv2d(x, f, 3, 1, 'same', kernel_initializer=init, name=f'conv{lvl}aa', reuse=tf.AUTO_REUSE)
                    x = tf.nn.leaky_relu(x, alpha=0.1)  # , name=f'relu{lvl+1}aa')
                    x = tf.layers.conv2d(x, f, 3, 1, 'same', kernel_initializer=init, name=f'conv{lvl}b', reuse=tf.AUTO_REUSE)
    ...
    def deconv(self, x, lvl, name='up_flow'):
        ...
        with tf.variable_scope('upsample'):
            ...
            return tf.layers.conv2d_transpose(x, 2, 4, 2, 'same', name=op_name, reuse=tf.AUTO_REUSE)
    ...
    def refine_flow(self, feat, flow, lvl, name='ctxt'):
        ...
        init = tf.keras.initializers.he_normal()
        with tf.variable_scope(name):
            x = tf.layers.conv2d(feat, 128, 3, 1, 'same', dilation_rate=1, kernel_initializer=init, name=f'dc_conv{lvl}1', reuse=tf.AUTO_REUSE)
            x = tf.nn.leaky_relu(x, alpha=0.1)  # default alpha is 0.2 for TF
            x = tf.layers.conv2d(x, 128, 3, 1, 'same', dilation_rate=2, kernel_initializer=init, name=f'dc_conv{lvl}2', reuse=tf.AUTO_REUSE)
            x = tf.nn.leaky_relu(x, alpha=0.1)
            x = tf.layers.conv2d(x, 128, 3, 1, 'same', dilation_rate=4, kernel_initializer=init, name=f'dc_conv{lvl}3', reuse=tf.AUTO_REUSE)
            x = tf.nn.leaky_relu(x, alpha=0.1)
            x = tf.layers.conv2d(x, 96, 3, 1, 'same', dilation_rate=8, kernel_initializer=init, name=f'dc_conv{lvl}4', reuse=tf.AUTO_REUSE)
            x = tf.nn.leaky_relu(x, alpha=0.1)
            x = tf.layers.conv2d(x, 64, 3, 1, 'same', dilation_rate=16, kernel_initializer=init, name=f'dc_conv{lvl}5', reuse=tf.AUTO_REUSE)
            x = tf.nn.leaky_relu(x, alpha=0.1)
            x = tf.layers.conv2d(x, 32, 3, 1, 'same', dilation_rate=1, kernel_initializer=init, name=f'dc_conv{lvl}6', reuse=tf.AUTO_REUSE)
            x = tf.nn.leaky_relu(x, alpha=0.1)
            x = tf.layers.conv2d(x, 2, 3, 1, 'same', dilation_rate=1, kernel_initializer=init, name=f'dc_conv{lvl}7', reuse=tf.AUTO_REUSE)
